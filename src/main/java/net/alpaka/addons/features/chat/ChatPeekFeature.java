 ...
 * display mode of the HUD draw), the HUD chat hook in Gui, the overlay hook in Screen and the scroll
...
public final class ChatPeekFeature {
    ...
    public static boolean isPeekingInMenu() {
        Minecraft mc = Minecraft.getInstance();
        return mc.screen != null && isPeeking();
    }
    ...
    public static boolean onScroll(double yOffset) {
        ...
        mc.gui.getChat().scrollChat((int) amount);
        return true;
    }
